...
// ВАЖНО: Используем DeepSeek для перевода, так как Gemini давал сбои
    ...
    try {
        const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                "messages": [
                    { role: "system", content: "Translate the following text to English for an image generation prompt. Return ONLY the English translation." },
...
// --- 2. ГЕНЕРАЦИЯ (FLUX BUFFER) ---
async function generateImageBuffer(prompt) {
    try {
        const seed = Math.floor(Math.random() * 1000000);
        // Pollinations (Flux) -> Скачиваем как файл
